...
{
    [Header("Refs")]
    public Transform firePoint;                    // Ponto de disparo da arma
    [SerializeField] GameObject bulletPrefab;    // Prefab da bala padrão
    [SerializeField] Transform cam;              // Transform da câmara (ou mira)
    [SerializeField] ParticleSystem muzzleFlash; // Partículas do disparo
    [SerializeField] AudioSource fireAudio;      // Áudio de disparo

    [Header("Input")]
    [SerializeField] InputActionReference shootAction;   // Ação de disparo
    [SerializeField] InputActionReference reloadAction;  // Ação de reload

    [Header("Settings (fallbacks se não houver config)")]
    [SerializeField] float bulletSpeed = 40f;    // Velocidade da bala
    [SerializeField] float fireRate = 0.12f;     // Cadência de disparo
    [SerializeField] float maxAimDistance = 200f; // Distância máxima de mira

    [Header("Behaviour")]
    [Tooltip("Player: TRUE (só dispara com WeaponConfig). Bot: FALSE (usa campos locais).")]
    [SerializeField] bool requireConfigForFire = true;

    WeaponConfig[] allConfigs;                    // Todas as configs disponíveis no objeto
    WeaponConfig activeConfig;                    // Config ativa atualmente
    Component weaponSwitcher;                     // Componente de troca de armas
    CharacterController playerCC;                // Referência ao CharacterController do jogador
    private bool isBot = false;                  // Flag para bots
    private Health ownerHealth;                  // Componente Health do jogador
    private PlayerShield playerShield;           // Componente Shield do jogador

    float nextTimeUnscaled;                       // Tempo do próximo disparo
    class AmmoState { public int inMag; public int reserve; } // Estado de munição
    readonly Dictionary<WeaponConfig, AmmoState> ammoByConfig = new();
    int currentAmmo, reserveAmmo;                // Ammo actual e reserva
    bool isReloading;                             // Flag de reload
    ...
    void Start()
    {
        if (isBot)
        {
            EnableInputsAndHUD(true); // Bots usam inputs automáticos
        }
    }
    ...
    public override void OnNetworkSpawn()
    {
        if (!IsOwner && !isBot)
        {
            ...
            this.enabled = false; // Desativa script se não for dono
            return;
        }
    ...
    void Update()
    {
        ...
        // Desativa inputs se o jogador estiver morto, pausado ou protegido
