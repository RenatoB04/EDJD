...
void findMaxSum(ED* matrix, int n, int m, int row, int col, int sum,
                bool* usedRows, bool* usedCols) {
  if (row == n) {
    if (sum > maxSum) {
      maxSum = sum;
      for (int i = 0; i < n; i++) {
  ...
  for (int j = 0; j < m; j++) {
    if (!usedCols[j] && !usedRows[row]) {
      ...
      usedCols[j] = true;
      usedRows[row] = true;
      findMaxSum(matrix, n, m, row + 1, 0, sum + node->data, usedRows,
                 usedCols);
      usedCols[j] = false;
      usedRows[row] = false;
    }
